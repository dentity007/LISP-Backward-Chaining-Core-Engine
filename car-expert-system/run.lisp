...
(defun consultation-menu ()
  ...
  (format t "5. Toggle tracing (~A)~%" (if *trace-enabled* "ON" "OFF"))
  (format t "6. System info~%")
  (format t "7. Exit~%")
  (format t "~%Choose an option (1-7): ")
  (let ((choice (read)))
    (case choice
      ...
      (7 (format t "Goodbye!~%"))
      (otherwise 
       (format t "Invalid choice. Please select 1-7.~%")
       (consultation-menu)))))
...
(format t "  (toggle-tracing) - Toggle detailed tracing~%")
